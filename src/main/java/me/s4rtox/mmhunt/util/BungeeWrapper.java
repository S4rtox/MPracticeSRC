 ...
 *
 */
public class BungeeWrapper implements PluginMessageListener {

    // The instance for this class, because there should only ever be one.
    private static BungeeWrapper com;
     ...
     */
    public BungeeWrapper(Plugin plugin) {
        com = this;
        this.plugin = plugin;
    ...
    private void receivePlayerCount(String server, int count) {
        com.playerCount.put(server, count);
    }
    ...
    public static int getPlayerCount(String server) {
        return com.playerCount.getOrDefault(server,0);
    }
    ...
    private static void out(Channel subChannel, Player p, String... args) {
        ...
        if(p != null) p.sendPluginMessage(com.plugin, "BungeeCord", out.toByteArray());
    }
